    ...
    return <div className='siteDetailsPage'>
      ...
      <div className={cx({
        ...
        <div>
          ...
          &nbsp;
          <a className={css(commonStyles.linkText)} href={`https://github.com/brave/browser-laptop/releases/tag/v${this.state.versionInformation.get('Brave')}dev`} target='_blank' data-l10n-id='relNotesInfo2' />
          &nbsp;
        ...
        <SortableTable
          ...
          rows={this.state.versionInformation.map((version, name) => [
            ...
            {
              html: name === 'rev'
                ? <a target='_blank' className={css(commonStyles.linkText)} href={`https://github.com/brave/browser-laptop/commit/${version}`}>{(version && version.substring(0, 7)) || ''}</a>
                : version,
              value: version
            }
          ])}
